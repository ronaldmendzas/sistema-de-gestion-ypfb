...
def render():
    ...
    with tab_er:
        ...
        st.markdown("---")
        st.info("Nota: La tabla DESPACHOS contiene columnas operativas (id_despacho, id_pedido, id_plantas, litros_despachados, fecha_despacho, placa_cisterna) y columnas financieras (costo_importacion_real, subvencion_asumida_bs). Esta dualidad es la base de la fragmentacion hibrida.")
    ...
    with tab_frag:
        ...
        with tab_s:
            ...
            st.caption("Cada nodo almacena solo los registros de su planta. La reconstruccion se realiza con UNION (∪).")
            ...
            st.code("STOCK_PLANTAS_Global = PC1 ∪ PC2 ∪ PC3", language="sql")
        ...
        with tab_p:
            ...
            st.code("PEDIDOS_WEB_Global = PC1 ∪ PC2 ∪ PC3", language="sql")

        with tab_d:
            st.markdown("#### Fragmentacion Hibrida — DESPACHOS")
            st.caption("Corte vertical (separacion de columnas confidenciales) + Corte horizontal (distribucion por planta).")
            ...
            with c3:
                ...
                st.dataframe(pd.DataFrame({"id_despacho": [15001, 15002, 25001, 25002, 35001], "costo_real": [127500, 170000, 102000, 153000, 212500], "subvencion_bs": [71400, 95200, 57120, 86040, 119000]}), hide_index=True)
        ...
        with tab_r:
            ...
            st.markdown("**FASE 1: Extraccion** — Cada nodo ejecuta su consulta SQL local optimizada.")
            st.code("""df_LP   = pd.read_sql('SELECT * FROM despachos_lp', conn_psycopg2_LP)
df_CBBA = pd.read_sql('SELECT * FROM despachos_cbba', conn_pymysql_CBBA)
df_SC   = pd.read_sql('SELECT * FROM despachos_sc', conn_psycopg2_SC)""", language="python")
            ...
            st.code("""df_finanzas = pd.read_sql('SELECT * FROM despachos_finanzas', conn_psycopg2_SC)
df_final = pd.merge(df_operativos, df_finanzas, on='id_despacho', how='inner')""", language="python")

            st.markdown("---")
            st.success("Resultado: DESPACHOS_Global = DESPACHOS_OPERATIVOS_Global ⋈ DESPACHOS_FINANZAS")
            st.caption("Columnas operativas (azul #003366) + Columnas financieras (verde #166534) = Tabla completa reconstruida al 100%.")